...
2. **📄 带图片长文档生成工具** (`image_document_generator`) - 专业长篇文档智能生成器（支持图片）
3. **📋 模板分类工具** (`template_classifier`) - 智能判断文档类型并分类处理
4. **🖼️ 图片RAG工具** (`image_rag_tool`) - 图片上传和基于文本描述的图片检索
5. **🎯 专业文档工具** (`professional_document_tool`) - 完整的RAG+AI智能文档处理功能
...
| 📄 带图片长文档生成工具 | 长篇专业文档生成（含图片） | 技术报告、研究文档 | **多阶段生成、图片检索插入、任务管理** |
| 📋 模板分类工具 | 智能文档类型识别 | 文档上传分类处理 | **自动分类、模板库管理、RAG集成** |
| 🖼️ 图片RAG工具 | 图片上传和检索 | 图片知识库管理 | **用户描述、向量化存储、文本检索** |
...
# 系统自动识别为模板文档，保存到templates_storage文件夹

file_path = "uploads/技术规范文档.pdf"  
# 系统自动识别为资料文档，调用RAG工具进行embedding处理
```
...
```
用户需求：生成包含现场图片的技术报告
选择工具：image_document_generator
处理流程：创作指令分析 → 大纲生成 → 章节内容 → 图片检索插入 → DOCX输出
输出结果：图文并茂的专业技术报告
```
...
```
用户操作：上传工程现场照片并提供描述
系统处理：用户描述 → 向量化存储 → 文本检索
应用场景：基于描述搜索相关图片、图片资料整理
```
